    ...
    "agents": [
        {
            ...
            "role": "Main PM agent. Plan, split work, assign subagents, and coordinate external sync.",
...
def list_agent_definitions() -> list[AgentDefinition]:
    ...
    for raw in config.get("agents", []):
        ...
        command_text = os.getenv(raw.get("commandEnv", ""), "").strip() or raw.get(default_key, "")
        agents.append(
            AgentDefinition(
                id=raw["id"],
...
def worker_agent_ids() -> list[str]:
    main = main_agent_id()
    return [agent.id for agent in list_agent_definitions() if agent.id != main]


def all_agent_ids() -> list[str]:
    return [agent.id for agent in list_agent_definitions()]
